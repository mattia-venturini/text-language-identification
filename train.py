...
if __name__ == "__main__":
	...
	rnn = model.RNN(data.n_letters, n_hidden, data.n_categories)
	...
	for epoch in range(1, args.epochs + 1):
		...
		if epoch % print_every == 0:
			...
			print('%d %d%% (%s) %.4f %s / %s %s' % (epoch, epoch / args.epochs * 100, timeSince(start), loss, data.unicodeToAscii(line), guess, correct))
	...
	# testing
	predict(rnn, "Typical approaches are based on statistics of the most frequent n-grams in each language")
	predict(rnn, "Please see the solution details below and run the code yourself.")
	predict(rnn, "i file contengono una o più frasi separate da una riga vuota;")
	predict(rnn, "Hola, amigo, como estas?");
